    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-10">
        ...
        <form action="{{ route('admin.statistik.update') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
                ...
                <div class="space-y-4">
                    ...
                    <div class="relative group">
                        <input type="number" name="kecamatan[{{ $item->kecamatan }}]" 
            ...
            <div class="mt-12 pt-8 border-t border-slate-50 flex items-center gap-6">
                <button type="submit" 
                        class="bg-[#0F172A] text-white px-10 py-5 rounded-[1.5rem] font-bold hover:bg-slate-800 transition-all shadow-2xl shadow-slate-200 active:scale-95 flex items-center gap-3">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                    </svg>
                    SIMPAN PERUBAHAN
                </button>
                <a href="{{ route('admin.dashboard') }}" class="text-slate-400 font-bold text-sm hover:text-slate-600 transition">Batal</a>
            </div>
        </form>
    </div>
</div>
